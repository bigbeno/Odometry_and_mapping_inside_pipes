classdef (Sealed) HebiKinematics
    ...
    %      getAccelCompTorques  -  compensates joint accelerations
    %
    %   Example
    ...
    methods(Access = public)
        ...
        function out = getAccelCompTorques(this, varargin)
            % getGravCompTorques compensates joint accelerations
            %
            %   This method computes the torques that are required to
            %   accelerate the body masses as determined from the joint
            %   commands that are handed in.
            %
            %   'Positions' argument expects a vector of positions of
            ...
            %      % Compensate gravity at current position
            %      fbk = group.getNextFeedback();
            %      cmdPositions = rand(1,group.getNumModules);
            %      cmdVelocities = rand(1,group.getNumModules);
            %      cmdAccelerations = rand(1,group.getNumModules);
            %      torques = kin.getGravCompTorques(...
            %          fbk.position, ...
            %          cmdPositions, ...
            %          cmdVelocities, ...
            %          cmdAccelerations);
            %
            %   See also HebiKinematics
            out = getAccelCompTorques(this.obj, varargin{:});
        end
